...
class _FormOrderState extends State<FormOrder> {
  ...
  void initState() {
    ...
    super.initState();
    getMachine().then((val) => addPackage());
    // getCurrentLocation();
  }
  ...
  void addPackage() async {
    ...
    soMachine.forEach((en) {
      storedPackage.add(en['id_paket'] + "-" + en['nama_paket']);
    });
    setState(() {
      arrayPackage = List.from(getPket);
      print('IDPOKJA : ${widget.idPokja}');
    });
    print('$arrayPackage');
  }

  bool visibPaket = false;
  void addPackageWithMachine(String idMachine) async {
    storedPackage.clear();
    arrayPackage.clear();
    var idUser = await storageUser.getItem('data_user_login')['data']['id'];
    var split = idMachine.split("-");
    final http.Response response = await http.post(apiURI, body: {
      "id_korwil": idUser,
      "id_mesin": split[0],
      "target": 'user_korwil',
  ...
  void filterPrice(String idPaket) {
    ...
    setState(() {
      ...
      estimasiHarga.text = pricePackage.toStringAsFixed(0);
      // print('${store[0]['harga']}');
    });
  }
  //PAKET SELECT ENDS
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: CustomAppBar(
          label: 'Jadwalkan order ${widget.nama}',
        ),
        body: ListView(
          ...
          children: [
                  ...
                  child: Padding(
                    ...
                    child: DropdownButton(
                      ...
                      onChanged: (value) {
                        ...
                        addPackageWithMachine(value.toString());
                      },
                    ),
                  ),
                ),
            ...
            Container(
                ...
                width: 500,
                child:
                    const Text('Waktu Pekerjaan', textAlign: TextAlign.start)),
            Container(
            ...
            Visibility(
                visible: false,
                child: InputTextWhite(
                    label: 'Nama Lahan Plotting', controller: namaLahan)),
            Visibility(
              visible: false,
              child: Container(
                  padding: const EdgeInsets.only(bottom: 5, top: 20),
            ...
            const SizedBox(height: 10),
            InputTextWhite(
                label: 'Estimasi Luas (m2)',
                controller: estimasiLuas,
                obscuring: false,
                inputKeyboard: TextInputType.number,
                onchanged: () {
                  FocusScope.of(context).requestFocus(FocusNode());
                  double hasil = double.parse(pricePackage.toString()) *
                      (double.parse(estimasiLuas.text) / 10000);
                  setState(() {
                    estimasiHarga.text = hasil.toStringAsFixed(0);
                  });
                },
                readonly: false),
            InputTextWhite(
