...
import 'package:terra_korwil/pages/trans_cost_plot.dart';
import 'package:terra_korwil/theme/config.dart';
import 'package:terra_korwil/theme/stateless.dart';

class TransFromPlot extends StatefulWidget {
  final String list;
  const TransFromPlot({Key? key, required this.list}) : super(key: key);

  @override
  _TransFromPlotState createState() => _TransFromPlotState();
}

class _TransFromPlotState extends State<TransFromPlot> {
  ...
  Future setValue() async {
    List koor_polygon = jsonDecode(data['polygon'].toString());
    ...
    if (response.statusCode == 200) {
      ...
      if (status['status'] == 'success') {
        ...
        showDialog(
          ...
          context: context,
          builder: (BuildContext context) {
            return const DialogStatus(tipe: 'fail', label: 'Transaksi gagal');
          },
        );
      }
    } else {
  ...
  Future inputCost() async {
    ...
    var split2 = _stringIDMachine!.split('-');
    var idPokja = data['id_pokja'];
    var idCustomer = data['id_customer'];
    if (polylineLatLng.isEmpty) {
      ...
      Navigator.of(context).push(MaterialPageRoute(
        builder: (BuildContext context) => TransCostPlot(
            idUser: idUser,
